...
const {mouseCoordinates,close,hotspotID,ModalIsOpened,setModalIsOpened,ImagesModalisOpen,setImagesModalisOpen,setIsOpen} = UseModal()	
...
const getItem =Number(JSON.parse(localStorage.getItem('project_id')))
...
<FirstModal type={isEditing || isTouring  ? 'regular' : 'invisible'} onClick={()=>{setModalIsOpened(false); }} ref={isEditing || isTouring ?  ref : null} style={(ModalIsOpened?{top:`${mouseCoordinates.y+40}px`,  left:`${mouseCoordinates.x-90}px`} : {top:`${coords.y+40}px`,  left:`${coords.x-90}px`}) }  >
...
<Button>
  ...
  <NavLink onClick={()=>setIsOpen(false)} to={`/projects/${getItem}/tour/${data[0]?.tour}`}>
    Go to next Room
  </NavLink>
</Button>
